...
## Use fixed `imagecorruptions-imaug` version 1.1.5 [???](???)
The minimal supported `imagecorruptions-imaug` version was increased from `1.1.3` to `1.1.5`.
Version `1.1.4` removed the dependency on the unmaintained `pkg_resources` package. Yet itself suffered
from a logic error in the mitigation code, which made version `1.1.5` strictly necessary.
