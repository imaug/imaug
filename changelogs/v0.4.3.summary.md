...
## Skip test `TestBatchLoader` for with Python 3.14 [gh#26](https://github.com/imaug/imaug/pull/26)
Test for depreciated `BatchLoader` does not work on Linux for Python 3.14. `BatchLoader` will
be removed in next major release. Thus, the test is skipped.
...
## Mitigate NumPy 2.4 removal of non-ndim 0 to scalar [cb#15](https://codeberg.org/imaug/imaug/pulls/15)
...
the support to convert an higher dimensional array to scalar. There were two spots in this libary where it was used (in `imgaug/augmenters/size.py` and `imgaug/augmenters/pillike.py`). Both were fixed with minimal code changes based on suggestions via GitHub issues.
...
## Spot check documentation [cb#14](https://codeberg.org/imaug/imaug/pulls/14)
* Change `imgaug` to `imaug` when the package name is used. 
* Remove out-dated Anaconda installation.
* Correct PiPy installation instructions.
* Correct GitHub installation instructions.
* Change links to Jupyter examples to `imaug/imaug`.
